...
def main() -> int:
    ...
    if event == "UserPromptSubmit":
        messages = [
            ...
            "For a paper main-line, research-question, or title request, call language_assist spine_action=plan before drafting: lift the local case into a macro problem, propose a unifying method and cross-context predictions/falsifiers, then preserve five unranked macro/meso/local title candidates for the user. Run the exact-method collision search only after that formation; nearby work should sharpen the mechanism or generality target, never force a retreat to a tiny non-creative question. Register the selected spine and bind the canonical collision receipt before claiming the method is clear. "
            ...
            " For resource-sensitive multi-stage work, call research_design resource_plan_action=inventory and then plan a main-agent-selected serial DAG; simple one-response work is exempt. Do not infer network access, estimates, or a whole-task deadline. A managed_standard stage linked to a fresh user-selected reproducibility plan may use resource_plan_action=execute, which delegates to the existing integrity executor and records its guard telemetry; never complete that linked task from caller-reported telemetry. After an interruption, execute may reconcile an already persisted valid final receipt without rerunning the command; if no final receipt exists, replay is forbidden. Record other stage artifacts through their existing owners."
            " Direction finding starts only after explicit user authorization: call research_design direction_action=plan to freeze the redacted local resource snapshot, then use managed reproducibility and strict novelty receipts for each current revision. Any method/protocol/range/tracked-file revision requires both checks again. Finalize exactly five unranked eligible directions with HTTPS literature links and leave the final choice to the user."
            " Before any work that would otherwise call an external LLM API, call research_design with delegation_action=plan. By default use exactly one available native entry/economy subagent with low reasoning and serial execution. If no native subagent is available, continue with the main agent locally; never silently fall back to an external API. External API use requires the user's explicit provider choice or a registered cross-provider protocol. Record a hash-bound delegation receipt, and never describe a same-host or same-model subagent as an independent multi-model reviewer."
        ]
