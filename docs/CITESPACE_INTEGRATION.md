...
Multi-dimension extension (2026-09-26, evening): two further live runs on the same corpus showcase the richer analysis dimensions — Reference co-citation (run `20260926T192058_3d44cf40`, 128 nodes / 429 exported edges) and Keyword (run `20260926T192212_8a6eb60d`, 94 nodes / 485 exported edges) — receipts `showcase_reference.json` / `showcase_keyword.json`. Display-window automation notes verified over the bundled JAB bridge: the merged display window title is `CiteSpace: Display Merged - (c) ...` (prefix-match it; exact match on the short prefix fails), its toolbar exposes 60 accessible elements including the named push button `Show/Hide Citation/Frequency Burst`, and clicking it round-trips without error — but on a run where burstness detection was never executed the toggle produces zero pixel change, so burst overlays require running CiteSpace burst detection first. Screenshots must foreground the window (`ShowWindow(SW_RESTORE)` + `SetForegroundWindow`) before grabbing its rect, or the capture returns whatever window is on top.
