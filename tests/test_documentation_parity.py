...
class DocumentationParityTests(unittest.TestCase):
    def test_repository_contract_passes_and_audits_shared_image(self) -> None:
        ...
        self.assertEqual(report["pair_count"], 13)
        self.assertEqual(report["translation_files"], 13)
        ...
        self.assertIn("local-storage-maintenance", {item["id"] for item in report["pairs"]})
        self.assertIn("p28-network-channel", {item["id"] for item in report["pairs"]})
        self.assertIn("preset-audit", {item["id"] for item in report["pairs"]})
        self.assertIn("p29-preset-audit", {item["id"] for item in report["pairs"]})
