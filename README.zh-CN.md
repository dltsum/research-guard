...
| “核验全文公式。” | Lean 逻辑、Pint 量纲、SymPy 等价、Z3 可满足性、数值/协议检查 | 五类结果分开报告；符号必须定义并实际使用；Lean 未运行只能报 `NOT_RUN` |
...
- **公式与数字。** Lean、Pint、SymPy、Z3 和数值行为是五个独立结果。数值测试还
  检查数值是否在论文注册协议内合法；未使用、未定义、非法或混淆参数不能取得
  最终 PASS。
